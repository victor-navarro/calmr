...
#The options
opts <- get_model_opts(iterations = 10)
#The arguments
my_mod_args <- make_model_args(design = parse_design(des_df), pars = pars, opts = opts)
...
Although the `run_model` function returns a list with 4 tibbles, we only care about one of them: `rs` (the model responses). With that in hand, we can write our model function.
...
my_model_function <- function(pars, model_args){
  ...
  mod_res = run_model(args = model_args)$rs
