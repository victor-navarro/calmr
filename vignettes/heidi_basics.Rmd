...
If you ever want to check if your trials string will work with the simulator, you can use `trialParser`.
...
```{r}
trialParser("AB/10AC")  #not specifying a number of AB trials. Bad practice!
trialParser("10AB(AB)(US)") #considering a configural cue for elements AB
...
The final ingredient is a list with simulation options. We can get a default one with `getHeidiOpts`, and modify them accordingly.

```{r}
my_opts <- getHeidiOpts()
my_opts
...
And we can finally simulate the model with `runHeidi`. By default, the function returns a list with the relevant data. Let's see.

```{r}
my_sim <- runHeidi(design_df = my_blocking,
                   param_df = my_pars,
                   options = my_opts)
...
But, we rarely want the raw data. If you are like me, you will want to see some plots. Let's use `makePlots` to do just that.

```{r}
my_plots = makePlots(my_sim)
my_plots$`Exp Split Vs`
my_plots$`Control Split Vs`
