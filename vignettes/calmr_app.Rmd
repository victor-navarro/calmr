...
If you do not want to deal with the programming side of `calmr` 
or simply want to simulate an experimental design to see what a 
...
Alternatively, you can install the [`calmr.app` companion package](https://github.com/victor-navarro/calmr.app) 
and launch the app via `calmr.app::launch_app()`.
...
* Named stimuli are specified within parentheses. For example, (US) implies that there is an element named "US" instead of a 
compound containing the elements "U" and "S".
...
It is important to note that whatever you set up here will interact with the 
"Create trial blocks" option in the *Options* tab in the sidebar (see ahead). 
Here's the full breakdown of combinations and their behavior:
...
but 2A/1B gets shuffled into 1 block containing two A trials and one B trial.
...
After parsing a valid design, you can set up the parameters for the experiment. 
The number of parameters will change as a function of the model. 
...
The parameterization of a model in `calmr` can sometimes differ from what appears in the literature.
The following table contains links to the documentation pages for each model (warning: equations).
...
A new button will appear after parsing the experiment.
...
After you do a new button will allow you to download the results as a spreadsheet.
...
(especially when using more obscure models).
...
The following sections describe additional options that I found useful.

### Options

Here we set the number of iterations to run the experiment for (important if model behavior 
