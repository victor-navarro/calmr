function [DA,ANCCR,PRC,SRC,NC,Rs,Delta,Mij,Mi,Eij,Ei,eventlog] =...
    calculateANCCR_v3(eventlog, T, alpha, k,samplinginterval,w,threshold,...
        ...
        if DA(jt)>=0
            ...
            if any(Iedge)
                R(Iedge,je) = R(Iedge,je) -...
                    alpha_r*R(Iedge,je).*((Delta(Iedge,jt)./numevents(Iedge)) ./ ...
                    sum((Delta(Iedge,jt)./numevents(Iedge))));
            else
                R(:,je) = R(:,je);
            end
        end
