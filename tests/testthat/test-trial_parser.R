test_that("trial_parser works", {
  ...
  expect_setequal(
    trial_parser("10AB(AB)>(US)")$unique_nominal_stimuli,
    c("A", "B", "AB", "US")
  )
  # same, but with functional stimuli
  expect_setequal(
    trial_parser("10AB(AB)>(US)")$unique_functional_stimuli,
    c("A", "B", "AB", "US")
  )
  # serious parsing of nominal stimuli
  expect_setequal(
    trial_parser("10A(US_a)/10A>(US_b)")$unique_nominal_stimuli,
    c("A", "US_a", "US_b")
  )
  # serious parsing of functional stimuli
  expect_setequal(
    trial_parser("10A(US_a)/10A(US_b)")$unique_functional_stimuli,
    c("A", "US")
  )
  # parsing of many complex stimuli into functional stimuli
  expect_setequal(
    trial_parser("10(A_a)(US_a)/10(A_b)(US_b)")$unique_functional_stimuli,
    c("A", "US")
  )
  # mixing up simple and complex stimuli
  expect_setequal(
    trial_parser("10(A_a)(US_a)/10B(US_b)")$unique_functional_stimuli,
    c("A", "B", "US")
  )
  # mixing up trial notation
  expect_setequal(
    trial_parser("10(A_a)>(US_a)/10(B)>(US_b)")$unique_functional_stimuli,
    c("A", "B", "US")
  )
