test_that("make_model_args works", {
  ...
  df = data.frame(Group = c("X", "Y"),
                  ...
                  R1 = c(TRUE, TRUE))
  pars = get_model_params(df, model = "HD2022")

  #works with data.frames
  minib_args = make_model_args(df, pars, model = "HD2022", opts = get_exp_opts(miniblocks = T))
  #works with parsed data.frames
  minib_args = make_model_args(parse_design(df), pars, model = "HD2022", opts = get_exp_opts(miniblocks = T))
  ...
  minib_args = make_model_args(parsed_df, pars, model = "HD2022", opts = get_exp_opts(miniblocks = T))
