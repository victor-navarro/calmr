test_that("make_heidi_args works", {
  #### Testing miniblocks WITH randomization ####
  df = data.frame(Group = c("X", "Y"),
                  P1 = c("10A/#10A/10B", "2AB/6A/6B/2#AB"),
                  R1 = c(TRUE, TRUE))
  pars = get_params(df, default_par = 0.2)
  #throw error if design is not parsed
  expect_error(make_heidi_args(df, pars))
  parsed_df = parse_design(df)
  minib_args = make_heidi_args(parsed_df, pars, opts = get_heidi_opts(miniblocks = T))
  #should generate 10 blocks for group X, each containing one of each trial type
  expect_true(all(sapply(1:3, function(x) sum(ceiling(which(minib_args$tps[[1]] == x)/3)) == sum(1:10))))
  ...
  expect_true(all(sapply(4:5, function(x) sum(ceiling(which(minib_args$tps[[2]] == x)/8)))== sum(1:2)))
  expect_true(all(sapply(c(1, 3), function(x) sum(ceiling(which(minib_args$tps[[2]] == x)/8)))== sum(rep(1:2, 3))))
  ...
  expect_equal(minib_args$tps[[1]], rep(1:3, 10))
  #should generate 2 blocks for group Y, in repeating order (4, 1, 1, 1, 3, 3, 5, and so on)
  expect_equal(minib_args$tps[[2]], rep(c(4, 1, 1, 1, 3, 3, 3, 5), 2))
