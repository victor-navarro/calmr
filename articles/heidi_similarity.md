...
inhibition paradigm $$A(US)/AX$$, an inhibitor (e.g., X) that has a
strong effect on behavior when presented will only have a weak effect on
behavior if weakly retrieved by a stimulus that has a strong association
with it (e.g., A).
...
the US (e.g., A after AX/X(US) or X(US)/AX).

### Reproducing the simulation presented in the paper

In the paper, the authors plot the similarity between the retrieved
...
.50, respectively, for both conditions.
...
easily generated by passing the $\left. A\rightarrow X \right.$
...
![](heidi_similarity_files/figure-html/unnamed-chunk-3-1.png)

#### Plotting the distribution of responding

This one is a little bit trickier because the figure in the manuscript
effectively contains many experiments varying the number of XA trials
before starting the first-order conditioning phase. To address this, we
run multiple simulations with different experimental designs.
...
![](heidi_similarity_files/figure-html/unnamed-chunk-6-1.png)

#### Some final notes

In their paper, Honey and Dwyer do not completely specify the rules for
choosing a reference value for the similarity calculation whenever more
than one nominal stimulus has been experienced. For example, in the
...
retrieved (i.e., $\left. A\rightarrow X \right.$) and conditioned
saliency values, we encounter the problem of having to choose among at
least two conditioned values (X_a or X_b). Although the authors of the
paper chose the saliency for the nominal X that was conditioned with the
US (i.e., X_b), they do not specify how this choice should be made in
other, less intuitive situations. As a way to avoid solving this issue,
the current implementation of the similarity rule uses the average of
the nominal stimuli as a reference value for the similarity calculation.
For this specific simulation case, this implementation reduces the
effect of similarity over the distribution of responding.
