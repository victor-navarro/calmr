...
parse_vs <- function(mod){
  df = tibble::enframe(apply(mod$vs, 1, function(x) as.data.frame(as.table(x))), name = "trial") %>%
...
#' @rdname heidi_helpers
parse_acts <- function(mod){
  combs = tibble::enframe(lapply(mod$combvs, function(x) as.data.frame(as.table(x))), name = 'trial') %>%
    dplyr::mutate(trial_type = mod$trial_names[mod$tps], phase = mod$phase, block_size = mod$block_size) %>%
    tidyr::unnest(.data$value) %>% dplyr::mutate(Var1 = as.character(.data$Var1),
                                           Var2 = as.character(.data$Var2)) %>%
    dplyr::rename(s1 = .data$Var1, s2 = .data$Var2, value = .data$Freq) %>% dplyr::mutate(act_type = 'comb')
  chains = tibble::enframe(lapply(mod$chainvs, function(x) as.data.frame(as.table(x))), name = 'trial') %>%
    dplyr::mutate(trial_type = mod$trial_names[mod$tps], phase = mod$phase, block_size = mod$block_size) %>%
    tidyr::unnest(.data$value) %>% dplyr::mutate(Var1 = as.character(.data$Var1),
                                           Var2 = as.character(.data$Var2)) %>%
    dplyr::rename(s1 = .data$Var1, s2 = .data$Var2, value = .data$Freq) %>% dplyr::mutate(act_type = 'chain')
  return(rbind(combs, chains))
}
...
parse_heidi_results <- function(raw_results){
  ...
  full_results = raw_results %>% dplyr::rowwise() %>%
    dplyr::mutate(vs = list(parse_vs(.data$mod_data)), acts = list(parse_acts(.data$mod_data)),
                  rs = list(parse_rs(.data$mod_data)), as = list(parse_as(.data$mod_data))) %>%
    dplyr::ungroup()
  return(list(vs = full_results %>% dplyr::select(.data$group, .data$vs) %>%
              ...
              acts = full_results %>% dplyr::select(.data$group, .data$acts) %>%
                tidyr::unnest(.data$acts) %>%
