...
#' @param ... Additional parameters passed to `stats::dist` and `stats::cor` (in the case of RSA) or `RSA` (in the case of RSATest).
...
RSA <- function(comparison, trials = "all", dist_method = "euclidean", corr_method = "spearman", ...){
  ...
  trialdists = lapply(trialmats, function(x) stats::dist(x, method = dist_method, ...))
  #calculate correlation among models
  modcorrs = stats::cor(data.frame(lapply(trialdists, function(x) as.numeric(x)),
                                   check.names = F), method = corr_method, ...)
  methods::new("CalmrRSA",
               corr_mat = modcorrs,
...
RSATest <- function(comparison, n_samples = 1e3, p = .95, ...){
  ...
  perm_cors = replicate(n_samples,
                        stats::cor(apply(dist_dat, 2, function(x) sample(x)),
                                   method = rsa@corr_method))
  #get critical values
  lc = apply(perm_cors, c(1, 2), function(x) stats::quantile(x, probs = (1-p)/2))
  uc = apply(perm_cors, c(1, 2), function(x) stats::quantile(x, probs = p+((1-p)/2)))

  #get significance
  smat = rsa@corr_mat < lc | rsa@corr_mat > uc

  methods::new("CalmrRSATest",
               RSA = rsa,
               sig_mat = smat,
               lower_crit = lc,
               upper_crit = uc,
